...
import {OpenBottomDrawer, ReactNativeConsoleLog} from "../api/react-native/ReactNativeApi";
import {AuthenticationClient, SignInRequest} from "../api/flatter-api/FlatterClient";
...
const SignIn = () => {
    ...
    const handleSubmit = async () => {
        ...
        const authClient = new AuthenticationClient(FlatterApiSettings);

        try{

            const x = await authClient.signIn(request);
        }catch (e){
            ReactNativeConsoleLog(`2, ${e}`)
        }
    ...
    return (
        <section
            class="mx-auto flex h-full animate-fade-in flex-col gap-10 overflow-y-scroll px-4 no-scrollbar md:w-[400px]">
